...
var urlSearch = `${baseUrl}search?part=snippet&regionCode=us&maxResults=10&`;
var urlVideos = `${baseUrl}videos?part=snippet&regionCode=us&maxResults=10&`;
var urlPlaylist = `${baseUrl}playlistItems?part=snippet&maxResults=5&`;
...
export const fetchData = async (type, filter) => {
  ...
  return await fetch(url)
    ...
    .then((data) => {
      if (data.error) {
        alert(
          'Youtube API daily quota limit exceeded it will reset after 24hrs',
        );
      } else {
        return data.items;
      }
...
export const fetchVerticalVideo = async () => {
  return await fetch(
    `${urlPlaylist}playlistId=PLIbLfYSA8ACNYCOaDWmj6EA1F1uS-pyVL&key=${youtube_api}`,
  )
    .then((res) => res.json())
    .then((data) => {
      if (data.error) {
...
export const fetchLogo = async (channelId) => {
  return await fetch(
    `${baseUrl}channels?part=snippet&id=${channelId}&fields=items%2Fsnippet%2Fthumbnails&key=${youtube_api}`,
  )
    .then((res) => res.json())
    .then((data) => {
      if (data.error) {
...
export const fetchVideoInfo = async (videoId) => {
  return await fetch(
    `${baseUrl}videos?part=statistics&id=${videoId}&key=${youtube_api}`,
  )
    .then((res) => res.json())
    .then((data) => {
      if (data.error) {
    ...
    await fetch(url[i])
      ...
      .then((data) => {
        if (data.error) {
          ...
          for (let j = 0; j < data.items.length; j++) arr.push(data.items[j]);
        }
      });
  }
  shuffleArray(arr);
